...
import { getDiscounts, addDiscount, editDiscount, removeDiscount } from '../../redux/slice/discountSlice';
...
const DiscountPage = () => {
  ...
  const handleEditDiscount = async (values) => {
    try {
      await dispatch(editDiscount({ id: editingDiscount.id, data: values })).unwrap();
  ...
  const fetchData = async () => {
    const result = await dispatch(getDiscounts()).unwrap();
    return {
      data: result.data,
      success: true,
      total: result.total,
